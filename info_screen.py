...
def load_sun_data():
    astral_home_city = LocationInfo('Home', 'USA', 'US/Eastern', 42.559645, -71.669871)
...
def day_progressbar_update():
    daystart_datetime = datetime(datetime.now().year, datetime.now().month, datetime.now().day, 00, 00)
    dayend_datetime = datetime(datetime.now().year, datetime.now().month, datetime.now().day, 23, 59, 59)
    daynow_datetime = datetime.now()
    daystart_unix = datetime_to_unix(daystart_datetime)
    dayend_adj_unix = (datetime_to_unix(dayend_datetime)) - daystart_unix
    daynow_adj_unix = (datetime_to_unix(datetime.now())) - daystart_unix
    daynow_percentage = (daynow_adj_unix / dayend_adj_unix) * 100

    day_progressbar['value'] = daynow_percentage
    day_progressbar_label['text'] = '{:05.2f}'.format(round(daynow_percentage, 2)) + '%  '
    root.after(1000, day_progressbar_update)
...
def moon_query():
    ...
    home = wgs84.latlon(42.559645 * N, 71.669871 * W, 128)
...
lcl_clock_frame = Frame(root,
                        ...
                        width='263',
                        height='60')
lcl_clock_frame.place(relx=0.0, rely=0.944)
...
utc_clock_frame = Frame(root,
                        ...
                        width='263',
                        height='60')
utc_clock_frame.place(relx=0.136, rely=0.944)
...
cal_frame.place(relx=0.0, rely=0.462)
...
sensor_frame.place(relx=0.0, rely=0.0)
...
sun_frame.place(relx=0.601, rely=0.0)

sun_table = Table(sun_frame,
              ["Event", "Today", "Tomorrow", "Delta"],
              column_minwidths=[200, 200, 200, 150],
              cell_foreground='#00c800',
              cell_anchor_list=['w', 'center', 'center', 'center'],
              stripped_rows=("black", '#1e1e1e'),
              header_foreground='black',
...
day_progressbar_frame = Frame(root,
                              highlightthickness=2,
                              highlightbackground='black',
                              highlightcolor='black')
day_progressbar_frame.pack(fill=BOTH, expand=True)
day_progressbar_frame.pack_propagate(0)
day_progressbar_frame.place(relx=0.0, rely=0.361)

day_progressbar_style = ttk.Style()
day_progressbar_style.configure("day.Horizontal.TProgressbar", background='#5f92fc')
day_progressbar = ttk.Progressbar(
    day_progressbar_frame,
    orient='horizontal',
    mode='determinate',
    length=382,
    style="day.Horizontal.TProgressbar")

day_progressbar.grid(column=0, row=2, columnspan=1, padx=10, pady=18)

day_progressbar_name = ttk.Label(day_progressbar_frame, text='Day Progress', font=('Charcoal', 22))
day_progressbar_name.grid(column=0, row=0, columnspan=1, pady=5)
day_progressbar_label = ttk.Label(day_progressbar_frame, text='', font=('Charcoal', 22))
day_progressbar_label.grid(column=1, row=2, columnspan=1)
...
year_progressbar_frame.place(relx=0.0, rely=0.261)
...
year_progressbar_name = ttk.Label(year_progressbar_frame, text='Year Progress', font=('Charcoal', 22))
year_progressbar_name.grid(column=0, row=0, columnspan=1, pady=5)
year_progressbar_label = ttk.Label(year_progressbar_frame, text='', font=('Charcoal', 22))
...
moon_table_frame = Frame(root,
                         ...
                         #width='602',
                         #height='400')
moon_table_frame.place(relx=0.272, rely=0.368)
...
moon_info_frame = Frame(root,
                        ...
                        highlightbackground='#00c800',
                        highlightcolor='#00c800')
moon_info_frame.place(relx=0.272, rely=0.0)
