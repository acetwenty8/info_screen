...
class Header_Cell(Cell):
    def __init__(self, master, text, bordercolor=None, borderwidth=0, padx=0, pady=0, background=None, foreground=None, font=None, anchor=CENTER, separator=True):
        #Cell.__init__(self, master, background=background, highlightbackground=bordercolor, highlightcolor=bordercolor, highlightthickness=borderwidth, bd= 0)
        Cell.__init__(self, master, background='black', highlightbackground=bordercolor, highlightcolor=bordercolor, highlightthickness=0, bd= 0)
        ...
        header_style.configure('header_style.TButton', width=0, relief='raised', background=background, foreground=foreground, font=font)
        self._header_label = ttk.Button(self, text=text, style='header_style.TButton')
        self._header_label.pack(padx=2, pady=2, fill=BOTH, expand=True, anchor='center')
        ...
        height = self._header_label.winfo_reqheight() + padx
        width = self._header_label.winfo_reqwidth() + pady
...
class Table(Frame):
    def __init__(self,
                 ...
                 scroll_vertically=None):
        outerborder_width = outerborder_width if outerborder else 0
        ...
        self._cell_anchor = []
        if cell_anchor_list is None:
            for j in range(len(columns)):
                self._cell_anchor.append(cell_anchor)
        else:
            for j in range(len(columns)):
                if 0 <= j < len(cell_anchor_list):
                    self._cell_anchor.append(cell_anchor_list[j])
                else:
                    self._cell_anchor.append(cell_anchor)
        ...
        self._head_enable = header_enable

        self._head = Frame(self, background='black', highlightbackground=bordercolor, highlightcolor=bordercolor, highlightthickness=outerborder_width, bd= 0)
        self._head.grid(row=0, column=0, sticky=E+W)

        header_separator = False if outerborder else True

        if self._head_enable is True:
            for j in range(len(columns)):
        ...
        for i in range(number_of_rows, number_of_rows+n):
            ...
            for j in range(number_of_columns):
                ...
                if self._stripped_rows:
                    cell = Data_Cell(self._body, borderwidth=self._innerborder_width, variable=var, bordercolor=self._bordercolor, padx=self._padx, pady=self._pady, background=self._stripped_rows[i%2], foreground=self._cell_foreground, font=self._cell_font, anchor=self._cell_anchor[j])
                else:
                    cell = Data_Cell(self._body, borderwidth=self._innerborder_width, variable=var, bordercolor=self._bordercolor, padx=self._padx, pady=self._pady, background=self._cell_background, foreground=self._cell_foreground, font=self._cell_font, anchor=self._cell_anchor[j])
